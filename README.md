# Poppy
I hate bloating my PC with the resource heavy apps like the NVIDIA app, just for the driver udpates. That's why I created `Poppy` - a Windows application that checks for NVIDIA graphics driver updates. It scrapes the web to fetch the latest driver version from the web and compares it with the current version installed on your system.
